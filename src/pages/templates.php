...
<svg aria-hidden="true" class="ui-icon-sprite">
  <symbol id="tpl-icon-template" viewBox="0 0 24 24">
    <path d="M5 4h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zm0 3v2h14V7H5zm0 4v7h7v-7H5z"
      fill="currentColor" />
  </symbol>
  <symbol id="tpl-icon-spark" viewBox="0 0 24 24">
    <path
  ...
  <symbol id="tpl-icon-eye" viewBox="0 0 24 24">
    <path
      d="M12 5c5.5 0 9.8 4.1 11 7-1.2 2.9-5.5 7-11 7S2.2 14.9 1 12c1.2-2.9 5.5-7 11-7zm0 2c-4.2 0-7.7 3-9 5 1.3 2 4.8 5 9 5s7.7-3 9-5c-1.3-2-4.8-5-9-5zm0 2.5a2.5 2.5 0 1 1-2.5 2.5A2.5 2.5 0 0 1 12 9.5z"
      fill="currentColor" />
  </symbol>
  <symbol id="tpl-icon-plus" viewBox="0 0 24 24">
    <path d="M11 5h2v6h6v2h-6v6h-2v-6H5v-2h6V5z" fill="currentColor" />
  </symbol>
  <symbol id="tpl-icon-bold" viewBox="0 0 24 24">
    <path d="M8 4h6a4 4 0 0 1 0 8H8V4zm0 10h7a4 4 0 0 1 0 8H8v-8zm4-8H10v4h2a2 2 0 0 0 0-4zm1 10h-3v4h3a2 2 0 0 0 0-4z"
      fill="currentColor" />
  </symbol>
  <symbol id="tpl-icon-italic" viewBox="0 0 24 24">
    <path d="M10 4v2h3.2l-4.4 12H6v2h8v-2h-3.2l4.4-12H18V4h-8z" fill="currentColor" />
  </symbol>
  <symbol id="tpl-icon-underline" viewBox="0 0 24 24">
    <path d="M8 4v7a4 4 0 0 0 8 0V4h-2v7a2 2 0 0 1-4 0V4H8zm-2 14h12v2H6v-2z" fill="currentColor" />
  </symbol>
  <symbol id="tpl-icon-strike" viewBox="0 0 24 24">
    <path
  ...
  <symbol id="tpl-icon-list-numbers" viewBox="0 0 24 24">
    <path
  ...
  <symbol id="tpl-icon-save" viewBox="0 0 24 24">
    <path d="M5 3h12l4 4v14H3V3h2zm2 2v5h8V5H7zm0 14h10v-7H7v7z" fill="currentColor" />
  </symbol>
</svg>

<section class="page-header page-header-create">
  <div>
    <div class="eyebrow"><svg class="icon">
        <use href="#tpl-icon-template"></use>
      </svg>Template Studio</div>
    <h1>Email Templates</h1>
    <p>Create reusable email templates, edit them with the same WYSIWYG editor, and generate polished HTML from a
      plain-text brief.</p>
  </div>
  <div class="header-meta">
    <a class="ghost" href="/index.php?page=create-campaign"><svg class="icon">
        <use href="#tpl-icon-plus"></use>
      </svg>New Campaign</a>
  </div>
</section>
  ...
  <aside class="templates-sidebar">
    <section class="card templates-list-card sticky-panel">
      <div class="templates-list-head">
        ...
        <a class="ghost" href="/index.php?page=templates"><svg class="icon">
            <use href="#tpl-icon-plus"></use>
          </svg>New</a>
      </div>
      ...
      <?php else: ?>
        <div class="templates-list">
          <?php foreach ($templates as $row): ?>
            ...
            <a class="template-list-item <?php echo $isActive ? 'active' : ''; ?>"
              href="/index.php?page=templates&template_id=<?php echo (int) $row['id']; ?>">
              <div class="template-list-item-top">
                <strong><?php echo htmlspecialchars((string) $row['name']); ?></strong>
                <span
                  class="status <?php echo status_badge_class((string) ($row['status'] ?? 'Active')); ?>"><?php echo htmlspecialchars((string) ($row['status'] ?? 'Active')); ?></span>
              </div>
      ...
      <section class="card section-card">
        <div class="section-card-head">
          <div>
            ...
            <p class="muted">Templates can be reused when composing campaigns. Include placeholders like
              `{{first_name}}` and `{{cta_url}}`.</p>
          </div>
          <div class="button-row">
            <?php if ($template): ?>
              <a class="ghost" href="/index.php?page=create-campaign">Use In Campaign</a>
            <?php endif; ?>
            <button class="button" type="submit" name="action" value="save_email_template"
              data-loading-text="Saving..."><svg class="icon">
                <use href="#tpl-icon-save"></use>
              </svg>Save Template</button>
          </div>
        </div>

        <div class="field-grid">
          <label class="field field-with-meta">
            <span class="field-label-row">Template Name
              <?php echo ui_info_popover('Internal name shown in the template library.'); ?></span>
            <input id="tplName" type="text" name="name" value="<?php echo htmlspecialchars($templateName); ?>"
              placeholder="e.g. Product Launch Announcement" required>
          </label>
          <label class="field field-with-meta">
            <span class="field-label-row">Category
              <?php echo ui_info_popover('Used for organization and filtering in the template library.'); ?></span>
            <input id="tplCategory" type="text" name="category"
              value="<?php echo htmlspecialchars($templateCategory); ?>" placeholder="Marketing / Product / Events">
          </label>
          <label class="field field-with-meta">
            <span class="field-label-row">Description
              <?php echo ui_info_popover('Visible in the template list to help teammates choose the right template.'); ?></span>
            <input id="tplDescription" type="text" name="description"
              value="<?php echo htmlspecialchars($templateDescription); ?>"
      ...
      <section class="card section-card">
        <div class="section-card-head">
          ...
          <div class="template-row">
            <div class="editor-mode-switch" role="tablist" aria-label="Editor mode">
              <button type="button" class="ghost active" data-tpl-editor-mode="visual"
            ...
            <button type="button" class="ghost" id="openTemplateAiModal"><svg class="icon">
                <use href="#tpl-icon-spark"></use>
              </svg>AI </button>
            <button type="button" class="ghost" id="openTemplatePreview"><svg class="icon">
                <use href="#tpl-icon-eye"></use>
              </svg> Preview</button>
          </div>
        </div>
        ...
        <div class="editor-toolbar" role="toolbar" aria-label="Template editor toolbar">
          <div class="toolbar-group">
            <button type="button" class="ghost icon-btn" data-command="bold" aria-label="Bold" title="Bold"><svg
                class="icon">
                <use href="#tpl-icon-bold"></use>
              </svg><span class="sr-only">Bold</span></button>
            <button type="button" class="ghost icon-btn" data-command="italic" aria-label="Italic" title="Italic"><svg
                class="icon">
                <use href="#tpl-icon-italic"></use>
              </svg><span class="sr-only">Italic</span></button>
            <button type="button" class="ghost icon-btn" data-command="underline" aria-label="Underline"
              title="Underline"><svg class="icon">
                <use href="#tpl-icon-underline"></use>
              </svg><span class="sr-only">Underline</span></button>
            <button type="button" class="ghost icon-btn" data-command="strikeThrough" aria-label="Strikethrough"
              title="Strikethrough"><svg class="icon">
                <use href="#tpl-icon-strike"></use>
              </svg><span class="sr-only">Strikethrough</span></button>
            <button type="button" class="ghost icon-btn" data-command="undo" aria-label="Undo" title="Undo"><svg
                class="icon">
                <use href="#tpl-icon-undo"></use>
              </svg><span class="sr-only">Undo</span></button>
            <button type="button" class="ghost icon-btn" data-command="redo" aria-label="Redo" title="Redo"><svg
                class="icon">
                <use href="#tpl-icon-redo"></use>
              </svg><span class="sr-only">Redo</span></button>
          </div>
          <div class="toolbar-group">
            <button type="button" class="ghost icon-btn" data-command="insertUnorderedList" aria-label="Bulleted list"
              title="Bulleted list"><svg class="icon">
                <use href="#tpl-icon-list-bullets"></use>
              </svg><span class="sr-only">Bulleted list</span></button>
            <button type="button" class="ghost icon-btn" data-command="insertOrderedList" aria-label="Numbered list"
              title="Numbered list"><svg class="icon">
                <use href="#tpl-icon-list-numbers"></use>
              </svg><span class="sr-only">Numbered list</span></button>
          </div>
          <div class="toolbar-group">
            <button type="button" class="ghost icon-btn" data-command="justifyLeft" aria-label="Align left"
              title="Align left"><svg class="icon">
                <use href="#tpl-icon-align-left"></use>
              </svg><span class="sr-only">Align left</span></button>
            <button type="button" class="ghost icon-btn" data-command="justifyCenter" aria-label="Align center"
              title="Align center"><svg class="icon">
                <use href="#tpl-icon-align-center"></use>
              </svg><span class="sr-only">Align center</span></button>
            <button type="button" class="ghost icon-btn" data-command="justifyRight" aria-label="Align right"
              title="Align right"><svg class="icon">
                <use href="#tpl-icon-align-right"></use>
              </svg><span class="sr-only">Align right</span></button>
            <button type="button" class="ghost icon-btn" data-command="justifyFull" aria-label="Justify"
              title="Justify"><svg class="icon">
                <use href="#tpl-icon-align-justify"></use>
              </svg><span class="sr-only">Justify</span></button>
          </div>
          <div class="toolbar-group">
          ...
          <div class="toolbar-group">
            <button type="button" class="ghost icon-btn" data-command="createLink" aria-label="Insert link"
              title="Insert link"><svg class="icon">
                <use href="#tpl-icon-link"></use>
              </svg><span class="sr-only">Insert link</span></button>
            <button type="button" class="ghost icon-btn" data-command="insertImage" aria-label="Insert image"
              title="Insert image"><svg class="icon">
                <use href="#tpl-icon-image"></use>
              </svg><span class="sr-only">Insert image</span></button>
            <button type="button" class="ghost icon-btn" data-command="removeFormat" aria-label="Clear formatting"
              title="Clear formatting"><svg class="icon">
                <use href="#tpl-icon-erase"></use>
              </svg><span class="sr-only">Clear formatting</span></button>
          </div>
          ...
          <button type="button" class="ghost icon-btn" id="tplInsertPlaceholder" aria-label="Insert placeholder"
            title="Insert placeholder"><svg class="icon">
              <use href="#tpl-icon-plus"></use>
            </svg><span class="sr-only">Insert placeholder</span></button>
        </div>
          ...
          <div class="editor-pane" id="tplVisualEditorPane">
            ...
            <div id="tplLinkInspector" class="link-inspector" hidden>
              <div class="link-inspector-url-row">
                ...
                <a id="tplLinkInspectorUrl" href="#" target="_blank" rel="noopener noreferrer"
            ...
            <div id="tplImageInspector" class="link-inspector image-inspector" hidden>
              <div class="link-inspector-url-row">
                ...
                <a id="tplImageInspectorSrc" href="#" target="_blank" rel="noopener noreferrer"
                  class="link-inspector-url">—</a>
              </div>
        ...
        <textarea id="tplHtmlContent" name="html_content"
          hidden><?php echo htmlspecialchars($initialHtml); ?></textarea>
        <textarea id="tplTextContent" name="text_content"
          hidden><?php echo htmlspecialchars($initialText); ?></textarea>
        <textarea id="tplSourceText" name="source_text"
          hidden><?php echo htmlspecialchars($templateSourceText); ?></textarea>
      </section>
      ...
      <?php if ($template): ?>
        <section class="card section-card danger-zone">
          <div class="section-card-head">
            ...
            <button class="ghost" id="deleteTemplateBtn" type="submit" name="action" value="delete_email_template"
              data-loading-text="Deleting...">Delete Template</button>
          </div>
        </section>
      <?php endif; ?>
    </form>
...
<dialog id="templateAiModal" class="preview-modal template-ai-modal">
  <div class="modal-head">
    <h3>AI Template Generator</h3>
    <button type="button" class="ghost" id="closeTemplateAiModal">Close</button>
  </div>
  <div class="modal-body">
    <p class="muted">Paste a brief or plain text email. Mailr will generate a polished HTML email template. Uses Ollama
      first, then Hugging Face, then optional OpenAI, then local fallback.</p>
    <div class="templates-ai-grid">
      <label class="field field-span-2">
        <span>Source Text / Brief</span>
        <textarea id="tplSourceTextEditor" rows="7"
          placeholder="Write the email brief, goals, offer, CTA, audience, and key points..."><?php echo htmlspecialchars($templateSourceText); ?></textarea>
        <span
          class="helper-inline"><?php echo ui_info_popover('Tip: include bullets and a CTA URL if you have one.'); ?></span>
        <span
          class="helper-inline"><?php echo ui_info_popover('Free AI options: run Ollama locally or use a Hugging Face Router token via MAILR_HF_API_KEY.'); ?></span>
      </label>
      <label class="field">
        <span>AI Tone</span>
        <select id="tplTone">
      ...
      <label class="field">
        ...
        <input id="tplBrand" type="text"
          value="<?php echo htmlspecialchars($brandDefault !== '' ? $brandDefault : 'Mailr'); ?>">
      </label>
      <label class="field">
    ...
    <div class="button-row">
      <button type="button" class="button" id="generateWithAi" data-loading-text="Generating..."><svg class="icon">
          <use href="#tpl-icon-spark"></use>
        </svg>Generate HTML</button>
    </div>
...
<script>
  (() => {
    ...
    const sourceTextEditor = document.getElementById('tplSourceTextEditor');
    const aiButton = document.getElementById('generateWithAi');
    const aiFeedback = document.getElementById('aiTemplateFeedback');
    const openAiModal = document.getElementById('openTemplateAiModal');
    const aiModal = document.getElementById('templateAiModal');
    const closeAiModal = document.getElementById('closeTemplateAiModal');
    ...
    const syncAiSourceField = () => {
      if (sourceText && sourceTextEditor) sourceText.value = sourceTextEditor.value;
    };
    ...
    if (aiButton) {
      aiButton.addEventListener('click', async () => {
        syncAiSourceField();
        const brief = sourceTextEditor ? sourceTextEditor.value.trim() : (sourceText ? sourceText.value.trim() : '');
        if (!brief) {
    ...
    if (sourceTextEditor) {
      sourceTextEditor.addEventListener('input', syncAiSourceField);
      sourceTextEditor.addEventListener('change', syncAiSourceField);
    }

    if (openAiModal && aiModal) {
      openAiModal.addEventListener('click', () => {
        if (sourceText && sourceTextEditor) {
          sourceTextEditor.value = sourceText.value || '';
        }
        if (aiFeedback) aiFeedback.hidden = true;
        if (aiModal.showModal) aiModal.showModal();
        else aiModal.setAttribute('open', 'open');
      });
    }
    ...
    if (closeAiModal && aiModal) {
      closeAiModal.addEventListener('click', () => {
        syncAiSourceField();
        if (aiModal.close) aiModal.close();
        else aiModal.removeAttribute('open');
      });
    }
